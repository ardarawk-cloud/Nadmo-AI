...
let balancePrivacyHidden=localStorage.getItem('nadmo_balance_hidden')!=='0';
const privateRupiah=n=>balancePrivacyHidden?'Rp ••••••••':rupiah(n);
const walletKey=n=>String(n||'').replace(/[\s_-]/g,'').toUpperCase();
const uid=()=>crypto.randomUUID?crypto.randomUUID():Date.now()+'-'+Math.random(); const now=()=>new Date().toISOString();
function privacyEye(){return balancePrivacyHidden?'<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M3 3l18 18M10.6 10.7a2 2 0 0 0 2.7 2.7M9.9 4.3A10.8 10.8 0 0 1 12 4c5.5 0 9 5 9 5a15.5 15.5 0 0 1-3.1 3.4M6.6 6.6C4.4 8.1 3 10 3 10s3.5 5 9 5c1 0 2-.2 2.9-.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>':'<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M3 12s3.5-6 9-6 9 6 9 6-3.5 6-9 6-9-6-9-6Z" stroke="currentColor" stroke-width="1.8"/><circle cx="12" cy="12" r="2.5" stroke="currentColor" stroke-width="1.8"/></svg>'}
function privacyToggle(){return `<button type="button" class="privacy-toggle" aria-label="${balancePrivacyHidden?'Tampilkan saldo':'Sembunyikan saldo'}" onclick="toggleBalancePrivacy()">${privacyEye()}</button>`}
...
function fmtAmount(el){let n=el.value.replace(/\D/g,'');el.dataset.raw=n;el.value=n?Number(n).toLocaleString('id-ID'):''} function selectChoice(el,id){document.querySelectorAll('#'+id+' .choice').forEach(x=>x.classList.remove('selected'));el.classList.add('selected')}
async function previewTx(type){let amount=Number((document.querySelector('#amount').dataset.raw||document.querySelector('#amount').value).replace(/\D/g,''));let desc=document.querySelector('#desc').value.trim();let c=document.querySelector('#catGrid .selected'),w=document.querySelector('#walletGrid .selected');if(!amount||!c||!w)return toast('Lengkapi nominal, kategori, dan wallet.');let cat=await get('categories',c.dataset.id),wal=await get('wallets',w.dataset.id);let d={type,amount,description:desc,categoryId:cat.id,categoryName:cat.name,walletId:wal.id,walletName:wal.name};modal(`<h2>Konfirmasi ${type==='expense'?'Pengeluaran':'Pemasukan'}</h2>${summary(d)}<button class="primary" onclick='saveTx(${JSON.stringify(JSON.stringify(d))})'>SIMPAN TRANSAKSI</button>`)}
function summary(d){return `<div class="card"><div class="summary-row"><span>Nominal</span><b>${rupiah(d.amount)}</b></div><div class="summary-row"><span>Deskripsi</span><b>${esc(d.description||'-')}</b></div><div class="summary-row"><span>Kategori</span><b>${esc(d.categoryName)}</b></div><div class="summary-row"><span>Wallet</span><b>${esc(d.walletName)}</b></div></div>`}
async function saveTx(raw){let d=JSON.parse(raw),old=state.editing?await get('transactions',state.editing):null;let ts=old?.timestamp||now();await put('transactions',{id:old?.id||uid(),timestamp:ts,type:d.type,amount:Number(d.amount),description:d.description,categoryId:d.categoryId,categoryName:d.categoryName,walletId:d.walletId,walletName:d.walletName,createdAt:old?.createdAt||now(),updatedAt:now()});state.editing=null;closeModal();toast('Transaksi tersimpan.');go('home')}
async function balance(){let raw=await all('wallets'),ws=[];for(const w of raw)ws.push({...w,balance:await walletBalance(w)});let total=(await totals()).balance;let sections=WALLET_GROUPS.map(([type,label])=>{let group=ws.filter(w=>w.type===type);if(!group.length)return'';let subtotal=group.reduce((n,w)=>n+w.balance,0);return `<div class="section-title wallet-balance-title"><span>${label}</span><span class="muted small">${privateRupiah(subtotal)}</span></div><div class="card">${group.map(w=>`<div class="wallet"><div><div class="wallet-name">${esc(w.name)}</div><div class="wallet-type">${esc(w.type)}</div></div><div class="wallet-bal ${w.balance<0?'expense':''}">${privateRupiah(w.balance)}</div></div>`).join('')}</div>`}).join('');return `<div class="app"><div class="pagehead"><button class="back" onclick="go('home')">‹</button><h1>Cek Saldo</h1></div><div class="hero privacy-hero"><div class="eyebrow">TOTAL SALDO</div>${privacyToggle()}<div class="balance privacy-balance">${privateRupiah(total)}</div><div class="muted small">Limit paylater/pinjaman tidak dihitung sebagai kekayaan.</div></div>${sections}${bottom()}</div>`}
async function history(){let tx=(await all('transactions')).sort((a,b)=>b.timestamp.localeCompare(a.timestamp)),f=state.filters,ws=await all('wallets'),cs=await all('categories');let start=new Date();if(f.period==='today')start.setHours(0,0,0,0);if(f.period==='week')start.setDate(start.getDate()-6),start.setHours(0,0,0,0);if(f.period==='month')start=new Date(start.getFullYear(),start.getMonth(),1);tx=tx.filter(t=>{let dt=new Date(t.timestamp),periodOk=f.period==='all'||f.period==='custom'||dt>=start;if(f.period==='custom'){let ds=t.timestamp.slice(0,10);periodOk=(!f.from||ds>=f.from)&&(!f.to||ds<=f.to)}return periodOk&&(!f.q||(`${t.description} ${t.categoryName} ${t.walletName}`).toLowerCase().includes(f.q.toLowerCase()))&&(!f.wallet||t.walletId===f.wallet)&&(!f.category||t.categoryId===f.category)});return `<div class="app"><div class="pagehead"><button class="back" onclick="go('home')">‹</button><h1>Riwayat</h1></div><div class="field searchbar"><input class="input" placeholder="Cari transaksi..." value="${esc(f.q)}" oninput="state.filters.q=this.value;debounceHistory()"><button class="iconbtn" onclick="go('history')">⌕</button></div><div class="pillrow">${[['today','Hari ini'],['week','Minggu ini'],['month','Bulan ini'],['custom','Custom Date'],['all','Semua']].map(x=>`<button class="chip ${f.period===x[0]?'on':''}" onclick="state.filters.period='${x[0]}';go('history')">${x[1]}</button>`).join('')}</div>${f.period==='custom'?`<div class="row" style="margin-top:10px"><input type="date" class="input" value="${f.from}" onchange="state.filters.from=this.value;go('history')"><input type="date" class="input" value="${f.to}" onchange="state.filters.to=this.value;go('history')"></div>`:''}<div class="row" style="margin-top:10px"><select class="select" onchange="state.filters.wallet=this.value;go('history')"><option value="">Semua Wallet</option>${ws.map(w=>`<option value="${w.id}" ${f.wallet===w.id?'selected':''}>${esc(w.name)}</option>`).join('')}</select><select class="select" onchange="state.filters.category=this.value;go('history')"><option value="">Semua Kategori</option>${cs.map(c=>`<option value="${c.id}" ${f.category===c.id?'selected':''}>${esc(c.name)}</option>`).join('')}</select></div><div class="section-title"><span>${tx.length} transaksi</span></div><div class="card">${tx.length?tx.map(txHtml).join(''):'<div class="empty">Tidak ada transaksi untuk filter ini.</div>'}</div>${bottom()}</div>`}
let histTimer;function debounceHistory(){clearTimeout(histTimer);histTimer=setTimeout(()=>go('history'),250)}
async function editTx(id){let t=await get('transactions',id);state.editing=id;modal(`<h2>Edit Transaksi</h2>${summary(t)}<div class="row"><button class="primary" onclick="closeModal();go('${t.type}')">Edit</button><button class="danger" onclick="deleteTx('${id}')">Hapus</button></div>`)} async function deleteTx(id){await del('transactions',id);state.editing=null;closeModal();toast('Transaksi dihapus, saldo terkoreksi.');go('history')}
async function transfer(){let ws=await all('wallets');return `<div class="app"><div class="pagehead"><button class="back" onclick="go('home')">‹</button><h1>Pindah Saldo</h1></div><div class="card"><div class="field"><div class="label">Dari</div><select id="from" class="select">${ws.map(w=>`<option value="${w.id}">${esc(w.name)}</option>`).join('')}</select></div><div class="field"><div class="label">Ke</div><select id="to" class="select">${ws.map(w=>`<option value="${w.id}">${esc(w.name)}</option>`).join('')}</select></div><div class="field"><div class="label">Nominal</div><input id="trAmount" class="input amount-input" inputmode="numeric" oninput="fmtAmount(this)" placeholder="Rp 0"></div><div class="field"><div class="label">Catatan</div><input id="trDesc" class="input" placeholder="Opsional"></div><button class="primary" onclick="saveTransfer()">SIMPAN TRANSFER</button></div>${bottom()}</div>`}
async function saveTransfer(){let from=document.querySelector('#from').value,to=document.querySelector('#to').value,a=Number((document.querySelector('#trAmount').dataset.raw||'').replace(/\D/g,''));if(from===to)return toast('Wallet sumber dan tujuan harus berbeda.');if(!a)return toast('Masukkan nominal transfer.');let fw=await get('wallets',from),tw=await get('wallets',to);await put('transfers',{id:uid(),fromWalletId:from,fromWalletName:fw.name,toWalletId:to,toWalletName:tw.name,amount:a,description:document.querySelector('#trDesc').value.trim(),timestamp:now(),createdAt:now()});toast('Transfer berhasil dicatat.');go('balance')}
async function settings(){let ws=await all('wallets'),cs=await all('categories');return `<div class="app"><div class="pagehead"><button class="back" onclick="go('home')">‹</button><h1>Pengaturan</h1></div><div class="section-title"><span>Wallet</span><button class="chip" onclick="walletEditor()">+ Tambah</button></div><div class="card">${ws.map(w=>`<div class="wallet" onclick="walletEditor('${w.id}')"><div><div class="wallet-name">${esc(w.name)}</div><div class="wallet-type">${esc(w.type)}</div></div><div class="wallet-bal">Saldo awal ${rupiah(w.initialBalance)}</div></div>`).join('')}</div><div class="section-title"><span>Kategori</span><button class="chip" onclick="categoryEditor()">+ Tambah</button></div><div class="card"><div class="muted small">${cs.length} kategori tersimpan. Tekan Kelola untuk edit/hapus.</div><button class="ghost" style="width:100%;margin-top:10px" onclick="categoryManager()">Kelola Kategori</button></div><div class="section-title"><span>Data & Backup</span></div><div class="card"><div class="grid"><button class="ghost" onclick="exportJSON(false)">Export JSON</button><button class="ghost" onclick="exportCSV()">Export CSV</button><button class="ghost" onclick="exportJSON(true)">Backup Database</button><button class="ghost" onclick="document.querySelector('#restoreFile').click()">Restore Backup</button></div></div>${bottom()}</div>`}
...
async function go(view,draft){state.view=view;if(view!=='expense'&&view!=='income')state.editing=null;await render(draft)} async function render(draft){let html;if(state.view==='home')html=await home();else if(state.view==='expense')html=await form('expense',draft);else if(state.view==='income')html=await form('income',draft);else if(state.view==='balance')html=await balance();else if(state.view==='history')html=await history();else if(state.view==='transfer')html=await transfer();else if(state.view==='settings')html=await settings();document.querySelector('#app').innerHTML=html;window.scrollTo(0,0)}
init().catch(e=>{document.querySelector('#app').innerHTML=`<div class="app"><div class="card">Gagal membuka database lokal: ${esc(e.message)}</div></div>`})
